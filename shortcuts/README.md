...
本目录提供自动配置协议 v1 的 [未签名通用模板](clipo-save.unsigned.shortcut)。它接收 Clipo 生成的一次性配置输入，领取服务器地址和设备 Token，保存为 iCloud Drive 的 `Shortcuts/Clipo.json`。平常运行时，有分享输入就使用分享内容，没有输入就读取剪贴板，提取第一个链接提交后台。适用于小红书、小黑盒只有“复制链接”的情况。

模板中没有个人地址和凭据；配置文件在首次运行时生成，可能随 iCloud 同步。不要分享配置文件，也不要把真实 Token 填回公共模板。旧的固定地址/Token 两动作版不支持自动配置，须替换为新版。

## 当前可用范围

服务端已提供配置码签发、领取、状态查询、取消和设备 Token 撤销；本地契约测试覆盖请求字段与生成一致性。设置页已提供“iPhone 快捷指令”区域，支持生成配置、唤起、复制配置文本、取消和领取状态。内置使用指南位于 `/shortcuts/`。本开发环境没有 Apple 签名工具，**未完成新模板的签名、iOS 导入、文件动作与系统通知验收**，不把未签名下载当成一键安装成品。
...
| Mac 签名、iOS 导入 | 待苹果设备验收 |
| 网页唤起与输入传递、服务器确认 | 待苹果设备验收 |
| `Shortcuts/Clipo.json` 的保存、读取和覆盖 | 待苹果设备验收；注意 iCloud 同步 |
| 剪贴板、系统分享、首个链接、无链接提示 | 待苹果设备验收 |
| 通知和 API 错误 | 待苹果设备验收；非 2xx 是否继续动作由系统决定 |
| 网络/TLS 错误 | 系统可能直接终止动作，自定义失败入口不保证执行 |

M4 仍待 iOS 实机验收，完整验证记录见 [构建进度](../docs/progress.md)。
