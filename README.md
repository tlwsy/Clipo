...
网页链接须使用 HTTP(S) 和 80/443 端口，拒绝内网地址。通用网页只支持公开静态 HTML；小红书支持 `/explore/帖子ID`、`/discovery/item/帖子ID` 和 `xhslink.com` 短链接，可在设置中填写 Cookie 后采集。小红书评论按采集上限保存 HTML 中已有的部分，不执行 JavaScript、不补抓分页；设置仅对后续执行的任务生效，已有笔记不变。Android 系统分享面板仍需真机验收。
