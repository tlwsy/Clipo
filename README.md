...
未配置模型或模型调用失败时，仍会保存原文，并显示“未生成摘要”。已进入 **Phase 3 平台专项**，首个节点提供小红书、小黑盒 Cookie 的加密配置与获取教程；平台抓取和登录有效性探测尚未接入。搜索、PWA 离线、扩展和备份按后续阶段推进。完整状态与验收限制见 [构建进度](docs/progress.md)。
...
网页链接须使用 HTTP(S) 和 80/443 端口；内网、需登录或依赖 JavaScript 的页面暂不支持。Android 系统分享面板仍需真机验收。
