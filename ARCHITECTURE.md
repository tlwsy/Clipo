...
7. 评论定稿    评分高于阈值（默认 0.6）标记 is_valuable
8. 媒体处理    下载缩略图；按设置决定是否下载原图
9. 落库        写入 notes / sources / comments / tags，更新任务状态为 success
```

当前 Phase 2 实现步骤 1–4、6 和 9；平台评论筛选、评分及媒体下载随后续阶段接入。暂时性抓取/处理失败：标记 `retrying`，重试 3 次（30s / 2min / 8min），最终置 `failed`。非法地址或不可访问内容直接失败；用户可在队列页手动重试。LLM 失败降级保存原文，任务仍为 `success`。
...
单次调用完成摘要与评论评分，减少费用与延迟。使用结构化输出（JSON Schema）约束返回：
...
Phase 2 输出摘要、要点与建议标签，评论评分在 Phase 3 接入。长正文按 token 预算保守截断（默认 8000，以 UTF-8 字节数作为上界，因此中文正文通常保留更少）。模型与 API Key 由用户在设置中配置，按用户隔离存储并加密于数据库。
