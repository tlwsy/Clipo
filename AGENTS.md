...
- Phase 4 的标签、收藏、中文检索、PWA 最近 50 篇离线缓存与写队列恢复已实现并通过本地验证；Shortcut 未签名模板已通过本地请求契约测试，按用户选择记录 Apple 签名、iOS 导入/分享/通知待实机验收，M4 尚未全部通过。Docker Compose 构建/启动/迁移/健康检查及 PostgreSQL 16 专项测试已通过，Compose 内完整浏览器业务链路仍待验收。浏览器扩展和备份仍属后续范围，`extension/` 当前只有说明文档。
