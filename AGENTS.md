...
- Phase 4 的标签、收藏、中文检索、PWA 最近 50 篇离线缓存与写队列恢复已实现并通过本地验证；Shortcut 已提供一次性配置码、设置页配置入口及支持剪贴板的未签名通用模板，已通过本地请求契约测试，按用户选择记录 Apple 签名、iOS 导入/分享/通知待实机验收，M4 尚未全部通过。Docker Compose 构建/启动/迁移/健康检查及 PostgreSQL 16 专项测试已通过，Compose 内完整浏览器业务链路仍待验收。浏览器扩展和备份仍属后续范围，`extension/` 当前只有说明文档。
...
- Commit 用于维持开发节奏、记录阶段内检查点，不能作为停止开发、结束当前任务或等待下一次指令的依据。
- 用户要求推进一个 Phase 时，应以该 Phase 的约定范围全部完成并通过相应验证作为停止开发的判断标准；每次节点提交后必须主动继续后续节点，不能因完成一次 commit 或单个节点就停下。
- Phase 完成前，只有确实需要用户提供材料或改变外部环境才能解除的阻塞，才说明原因并暂停依赖该阻塞的工作，其他可推进的工作继续完成；暂停不代表该 Phase 已完成。
