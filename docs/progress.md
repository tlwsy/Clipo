...
**Phase 3：平台专项，进行中。** 已实现小红书、小黑盒的 Cookie 配置与加密、小红书 HTML 帖子/内嵌评论采集和失败诊断，以及评论初筛、LLM 评分和阈值配置。接下来补齐可验证的 Cookie 有效性探测、评论分页与抓取上限配置、小黑盒和视频平台适配器。小红书真实平台及模型评分质量验收仍待完成，当前尚不满足 M3“关键平台可用”的验收标准。
