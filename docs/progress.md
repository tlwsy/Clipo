...
- 建议标签尚未变成可管理标签；通用网页不抓评论。小红书仅解析页面已有的最多 100 条顶层评论，不请求签名接口、评论分页或楼中楼。初筛与模型预算可能使评分条数少于采集条数，未评分不代表低价值；现有评分链路仅用固定模型响应验证，未验证真实模型评分质量。
...
**Phase 3：平台专项，进行中。** 已实现小红书、小黑盒的 Cookie 配置与加密、小红书 HTML 帖子/内嵌评论采集和失败诊断，以及独立的评论采集上限、评论初筛、LLM 评分和阈值配置。接下来补齐可验证的 Cookie 有效性探测、评论分页、小黑盒和视频平台适配器。小红书真实平台及模型评分质量验收仍待完成，当前尚不满足 M3“关键平台可用”的验收标准。
