...
- 评论采集上限：`capture.max_comments` 默认 100，可设为 0–100；目前仅作用于小红书内嵌顶层评论。关闭后新笔记不保存评论记录、模型不接收评论，帖子原文和 HTML 快照仍保存。独立于 `llm.max_comments`，提高上限不会误用较小采集范围的缓存，旧缓存兼容原有 100 条上限；设置与缓存均按账号隔离。
...
- 建议标签尚未变成可管理标签；通用网页不抓评论。小红书最多保存 100 条顶层评论，完整 Cookie 和访问参数齐备时补抓分页，不抓楼中楼。初筛与模型预算可能使评分条数少于采集条数，未评分不代表低价值；现有评分链路仅用固定模型响应验证，未验证真实模型评分质量。
...
**Phase 3：平台专项，进行中。** 已实现平台 Cookie 配置与加密、小红书 HTML 帖子/内嵌评论与签名分页、失败诊断、请求间隔及随机 UA，以及独立的评论采集上限、评论初筛、LLM 评分和阈值配置。接下来接入 Cookie 有效性探测入口、小黑盒和视频平台适配器。小红书已登录采集、签名分页及真实模型评分质量验收仍待完成，当前尚不满足 M3“关键平台可用”的验收标准；节点提交后继续推进本阶段。
