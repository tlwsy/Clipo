...
- `comments`：小红书适配器保存页面内嵌的顶层评论（最多 100 条，含作者、内容、点赞及回复数），不包含未加载的分页或楼中楼内容，尚未进行价值评分。通用网页提取器不提取评论。
...
Phase 3 继续接入独立 Cookie 有效性探测、评论分页/筛选/评分及其他平台适配器；Phase 4 接入 `q` 搜索、标签、收藏和 Shortcut；Phase 5 扩展 `POST /captures` 的内容直传；Phase 6 接入导出、导入与备份。分享链接、重新摘要、删除任务、限流等额外接口尚未实现，请勿依赖此前规划中的示例端点。
