...
当前 Phase 1–2 已实现基础、数据库、队列、通用网页提取和 LLM 配置；Phase 3 已提供平台 Cookie 的加密保存、替换与清除，小红书采集会使用当前账号的 Cookie。备份配置已实现，媒体下载仍为后续规划。实际部署模板见仓库根目录的 [`.env.example`](../.env.example)，执行 `make configure` 可生成带随机密钥的 `.env`。
...
模型会在同一次请求中生成摘要、要点、建议标签与候选评论评分。首次请求使用 JSON mode，失败后兼容普通 JSON 输出重试一次，再降级保存原文和全部已采集评论；评分无效但摘要有效时保留摘要，详情页单独提示评分失败。未评分显示为空，不作为 0 分或低价值判断。建议标签会自动归入可管理标签。
...
设置页“内容采集”的“评论采集上限”对应 `capture.max_comments`，默认 100，允许 0–100 的整数；设为 0 时不提取、保存或评分评论，但仍保存帖子正文、元数据和原始 HTML 快照。当前作用于小红书、小黑盒、B 站和 YouTube 的顶层评论，按平台响应顺序去除空内容和重复编号后计数；小红书有完整 Cookie 和带 `xsec_token` 的帖子链接时补抓分页，但不保证达到设置条数。
...
当前解析小红书 HTML 帖子并补抓顶层评论分页，最多 100 条；不抓楼中楼，采集成功本身不视为登录验证。分页需完整 Cookie（含 a1）和带访问参数的帖子链接，缺少时仅保存页面已有评论并显示提示。成功采集公开帖子也不能证明 Cookie 有效。独立 Cookie 检测已提供；Phase 5 浏览器扩展已提供 DOM 直取路径。用户提供的小红书/小黑盒样本已通过真实 Cookie 和模型采集链路；其他帖子类型仍需独立验收。
...
设置页可选 `none` / `local` / `s3` / `webdav`，计划为五段 Cron（留空仅手动）；S3 填 endpoint、bucket、region 和密钥，WebDAV 填已存在目录、用户名及应用密码。凭据加密保存，详情见[备份与恢复](backup.md)。

| 环境变量 | 默认 | 作用 |
| --- | --- | --- |
| `CLIPO_BACKUP_PATH` | 仓库 `data/backups/` | 本地备份根目录，按账号分隔；Docker `/app/data/backups/` |
| `CLIPO_BACKUP_KEEP` | `10` | 本地保留份数，1–100；远端生命周期由存储服务管理 |
| `CLIPO_EXPORT_RETENTION_DAYS` | `7` | 可下载副本的保留天数，1–365 |
| `CLIPO_BACKUP_ALLOWED_ORIGINS` | `[]` | 管理员允许的内网/自定义端口 origin JSON 数组，例如 `["http://minio:9000"]` |
| `CLIPO_TIMEZONE` | `Asia/Shanghai` | 定时备份采用的 IANA 时区 |

API 与 worker 须共享上述配置及队列所在目录。关闭备份目标会清除其已保存凭据；恢复笔记库无需旧密钥，恢复整个实例的配置则需要原主密钥。
...
B 站支持 BV/av 视频链接、b23.tv 与 `p` 分 P；YouTube 支持 watch、youtu.be、Shorts、live 和 embed 的单视频链接，不采集频道或播放列表。视频简介与可取得的一轨字幕共同保存到原文并用于摘要，字幕附语言/自动字幕标记。单轨最多保存 200000 字符，达到上限会提示。视频封面只保存外链，不下载媒体。

评论上限也适用于视频顶层热评，B 站最多 5 页，YouTube 最多 10 次接口请求，均检查 180 秒预算并控制请求间隔；0 关闭评论接口请求。缺少字幕、评论关闭、网络/登录/平台验证限制会保留已取得内容并显示原因。视频平台当前不接收 Cookie，不会使用小红书或小黑盒凭据；需要登录的字幕和受限制视频不能保证取得。YouTube 线上验收尚受当前网络 DNS 限制。

## iOS Shortcut 安装链接

`CLIPO_SHORTCUT_INSTALL_URL` 可设为已经在苹果设备验证的通用自动配置版 iCloud 链接（`https://www.icloud.com/shortcuts/` 加 32 位分享 ID）。模板须支持 Clipo 自动配置协议 v1，且不含个人服务器地址或 Token；旧的固定地址/Token 指令不兼容。默认留空，未发布通用版时不展示可用安装链接。更新后重启 API 或重建 Compose 应用；无需重新构建前端。

自动配置码有效期固定 5 分钟，一个账号同时保留一个最新码。创建时不签发 Token，设备成功领取后才创建可单独撤销的设备令牌。配置状态“已领取”不表示苹果设备完成文件保存，iOS 系统动作仍需实机验收。
