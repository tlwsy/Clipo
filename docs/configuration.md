...
设置页“内容采集”的“评论采集上限”对应 `capture.max_comments`，默认 100，允许 0–100 的整数；设为 0 时不提取、保存或评分评论，但仍保存帖子正文、元数据和原始 HTML 快照。当前仅作用于小红书的页面内嵌顶层评论，按页面顺序去除空内容和重复编号后计数；有完整 Cookie 和带 `xsec_token` 的帖子链接时补抓分页，但不保证达到设置条数。
